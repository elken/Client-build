...
def check_paths():
    for dep_name, val in paths.items():
        if not os.path.exists(val):
            if input("%s not found in expected location, "
                     "enter other path? [N/y] " % dep_name).lower() in ['yes',
                                                                        'y']:
                while True:
                ...
                if input("Install %s? [Y/n] " % dep_name).lower() in ['yes',
                                                                      'y', '']:
...
def install_cmake():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        get_file_with_progress("http://cmake.org/files/v3.3/" + file,
                               file,
                               False)

        subprocess.call(["cmake-3.3.2-win32-x86.exe",
                         "/S",
                         "/D=C:\Program Files (x86)\CMake"],
...
def install_boost():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        get_file_with_progress("http://sourceforge.net/projects/boost/files/la"
                               "test/download?source=files",
                               "boost.tar.bz2",
                               True)
...
def install_qt():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        get_file_with_progress("http://download.qt.io/official_releases/qt/5.5"
                               "/5.5.1/",
                               "qt-opensource-windows-x86-mingw492-5.5.1.exe",
                               "qt-opensource-windows-x86-mingw492-5.5.1.exe",
                               False)

        subprocess.call("qt-opensource-windows-x86-mingw492-5.5.1.exe",
                        stdout=subprocess.PIPE)
        os.chdir(old_dir)
...
def run_cmake():
    ...
    if not os.path.exists("CMakeLists.txt") or \
       not os.path.exists("../CMakeLists.txt"):
        if input("Would you like to download the "
                 "Project Ascension source? [Y/n] ").lower() in ["yes",
                                                                 "y",
                                                                 ""]:
            pa_cmake_dir = clone_repo()
        else:
            while True:
                pa_cmake_dir = input("Enter the path of the Project Ascension "
                                     "source code (with CMakeLists.txt in) ")
                if os.path.exists(os.path.join(pa_cmake_dir,
                                               "CMakeLists.txt")):
                    print("Found source code, building")
                    break
                else:
                    print("Directory is missing CMakeLists.txt, "
                          "please try again.")
    ...
    if lib_cmake_dir == "" or mingw_bin_dir == "" or qt_dll_dir == "":
        ...
        print("Running cmake -G \"MinGW MakeFiles\" -DWITH_TESTS=true "
              "-DCMAKE_PREFIX_PATH=\"%s\" %s"
              % (lib_cmake_dir, pa_cmake_dir))
        os.environ["PATH"] = paths["CMake"] + os.pathsep + mingw_bin_dir
...
def clone_repo():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        old_dir = os.getcwd()
        os.chdir(tmp_dir)
        get_file_with_progress("http://github.com/Proj-Ascension/Client/"
                               "archive/dev.zip", "dev.zip", True, clone_dir)
